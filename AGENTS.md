...
| `.chezmoitemplates/` | Shell env templates (`env.{bash,zsh,fish,ps1}-{linux,darwin,windows}.tmpl`) - generated by generate_env.py |
...
|-------|----------|-------------|
| `key` | yes | Variable name or alias name |
| `value` | yes | Value (supports `$VAR` expansion and chezmoi `{{ }}` templates) |
| `type` | no | `export` (default), `alias`, `eval`, `source`, `local` |
| `condition` | no | Shell condition dict (`{"type": "executable", "value": "brew"}`) or raw condition string |
| `os` | no | `"linux"`, `"darwin"`, `"windows"`, `"unix"` (alias for linux+darwin), or list like `["linux", "darwin"]` |
